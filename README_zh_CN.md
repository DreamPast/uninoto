...
当前项目有三个族，`sans`（无衬线）、`serief`（衬线）和`mono`（等宽），由Google Noto Fonts合并而来，CJK汉字优先使用**简体中文**字形。
...
`uninoto_last.ttf`包含了若干其余免费可商用后备字体，用于补充`sans`和`serief`。
...
截至Unicode17，一共有159563个可见字符，覆盖率如下：

- `sans`（包含`last`）：99.625%
- `serief`（包含`last`）：99.625%
- `mono`：86.833%
...
| 字体 | 用途 | 许可证 |
|------|------|--------|
| [BabelStone](https://www.babelstone.co.uk/Fonts/) | 多种古代文字、罕用字符 | OFL 1.1 |
...
合并输出的字体（`fonts/merged/uninoto_*.ttf`）是以 **SIL Open Font License, Version 1.1** 发布的复合/衍生作品，版权归属 DreamPast。

### 源字体许可证

所有源字体的完整许可证文本收录在仓库根目录的 [LICENSE](./LICENSE) 文件中，包括：

- **SIL Open Font License 1.1**（Noto 系列、BabelStone、Charis SIL、Doulos SIL、Junicode、Kanchenjunga、Kedebideri、Fairfax HD、Scriptwide Sans CJK、Cascadia Code 等）
